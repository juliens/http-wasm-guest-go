...
go_sources := $(wildcard examples/*/*.go) $(wildcard internal/test/testdata/*/*.go) $(wildcard internal/test/testdata/*/*/*.go) $(wildcard internal/test/testdata/*/*/*/*.go)
build.go: $(go_sources)
	@for f in $^; do \
	    GOOS=wasm GOARCH=wasi go build -o $$(echo $$f | sed -e 's/\.go/\.wasm/')  $$f; \
	done
